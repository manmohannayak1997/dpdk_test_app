...
void print_menu() {
    ...
    printf("  send tcp\n");
    printf("  display all\n");
    printf("  delete <src_ip> <dst_ip> <src_port> <dst_port> <proto>\n");
    printf("  flush\n");
    printf("  quit\n");
}
...
void send_udp() {
    ...
    printf("Parsed UDP packet: src_ip=%x dst_ip=%x src_port=%u dst_port=%u\n",
...
void send_tcp() {
    ...
    printf("Parsed TCP packet: src_ip=%x dst_ip=%x src_port=%u dst_port=%u\n",
        rte_be_to_cpu_32(ip->src_addr), rte_be_to_cpu_32(ip->dst_addr),
...
void display_all() {
    ...
    while (rte_hash_iterate(hash, &next_key, &next_data, &iter) >= 0) {
        struct packet_entry *e = (struct packet_entry *)next_data;
        printf("src_ip=%x dst_ip=%x src_port=%u dst_port=%u proto=%u\n",
            e->key.src_ip, e->key.dst_ip,
            rte_be_to_cpu_16(e->key.src_port),
            rte_be_to_cpu_16(e->key.dst_port),
            e->key.proto);
    }
}
